...
export def ReformMail(): void # ML の広告を削除する個人的な関数
	...
	elseif from ==? 'xtech-pcmobile@nikkeibp.co.jp'
		...
		silent :1 | :/^$/,/^$/+1delete | silent :%s/^　//ge
	elseif from ==? 'e_service@mof.go.jp' || from ==? 'sara.aihara@mof.go.jp'
		silent execute ':1 | :/当メールマガジンについてのご意見、ご感想はこちらへお願いします。/;$delete'
		silent :1 | silent :/^$/+1,$join! | :1 | silent :/^$/,$s/<\(br\|div\|p\|tr\)[^>]*>/\r/ge | :1 | silent :/^$/,$s/<[^>]\+>//ge | silent :%s/&nbsp;/ /ge | silent :%s/&hellip;/…/ge | :1 | silent :/^$/,$s/^[　 \t]\+//e | silent :%s/　/ /ge | silent :/\%^/,/^$/s/text\/\zshtml/plain/e | silent :%s/^\n\zs\n\+//e | silent :%s/&ldquo;/“/ge | silent :%s/&rdquo;/”/ge
	else
		return
	endif
	:1 | :/^$/,$Zen2han
