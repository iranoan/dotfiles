...
grep () { # --exclude/--include オプション両方が使われると、--include オプションが無視されるので分岐する
	include=0
	for arg in $@
	do
		case "$arg" in
